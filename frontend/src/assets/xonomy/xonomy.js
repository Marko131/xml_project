...
Xonomy.js2xml = function(js) {
  if (js.type == "text") {
    ...
    if (js.children.length > 0) {
      ...
      for (var i = 0; i < js.children.length; i++) {
        var child = js.children[i];
        if (child.type == "text") xml += Xonomy.xmlEscape(child.value);
        //text node
...
Xonomy.enrichElement = function(jsElement) {
  ...
  jsElement.getText = function() {
    ...
    for (var i = 0; i < this.children.length; i++) {
      ...
      else if (this.children[i].type == "element")
        txt += this.children[i].getText();
    }
    return txt;
  };
    ...
    if (parent) {
      ...
      for (var i = 0; i < parent.children.length; i++) {
        if (
          ...
          parent.children[i].htmlID != this.htmlID
        ) {
...
Xonomy.verifyDocSpec = function() {
  //make sure the docSpec object has everything it needs
...
Xonomy.asFunction = function(specProperty, defaultValue) {
  if (typeof specProperty == "function") return specProperty;
...
Xonomy.verifyDocSpecElement = function(name) {
  //make sure the DocSpec object has such an element, that the element has everything it needs
  if (
    !Xonomy.docSpec.elements[name] ||
    typeof Xonomy.docSpec.elements[name] != "object"
  ) {
    if (Xonomy.docSpec.unknownElement) {
      Xonomy.docSpec.elements[name] =
        typeof Xonomy.docSpec.unknownElement === "function"
  ...
  for (var attributeName in spec.attributes)
    Xonomy.verifyDocSpecAttribute(name, attributeName);
...
Xonomy.verifyDocSpecAttribute = function(elementName, attributeName) {
  //make sure the DocSpec object has such an attribute, that the attribute has everything it needs
  ...
  if (
    ...
    if (Xonomy.docSpec.unknownAttribute) {
      elSpec.attributes[attributeName] =
        typeof Xonomy.docSpec.unknownAttribute === "function"
...
Xonomy.verifyDocSpecMenuItem = function(menuItem) {
  //make sure the menu item has all it needs
  ...
  if (typeof menuItem.expanded != "function")
    menuItem.expanded = Xonomy.asFunction(menuItem.expanded, false);
};
...
Xonomy.refresh = function() {
  ...
  $(".xonomy .textnode").each(function() {
    //delete empty text nodes if the parent element is not allowed to have text
    var $this = $(this);
    ...
    if (
      elSpec &&
      !elSpec.hasText(Xonomy.harvestElement($parent.toArray()[0]))
    ) {
  ...
  $(".xonomy .element.hasText > .children > .element").each(function() {
    //determine whether each child element of hasText element should have empty text nodes on either side
    if (
      $(this).prev().length == 0 ||
      !$(this)
        .prev()
  ...
  $(".xonomy .attribute ").each(function() {
    //reorder attributes if necessary
    var atName = this.getAttribute("data-name");
    var elName = this.parentNode.parentNode.parentNode.getAttribute(
      "data-name"
    );
    var elSpec = Xonomy.docSpec.elements[elName];
    var mustBeAfter = [];
    for (var sibName in elSpec.attributes) {
      if (sibName == atName) break;
      else mustBeAfter.push(sibName);
    }
    var mustBeBefore = [];
    var seen = false;
    for (var sibName in elSpec.attributes) {
      if (sibName == atName) seen = true;
      else if (seen) mustBeBefore.push(sibName);
    }
    if (mustBeBefore.length > 0) {
      //is it after an attribute it cannot be after? then move it up until it's not!
      var $this = $(this);
      var ok;
      do {
        ok = true;
        for (var ii = 0; ii < mustBeBefore.length; ii++) {
          if (
            $this.prevAll("*[data-name='" + mustBeBefore[ii] + "']").toArray()
              .length > 0
          ) {
    ...
    if (mustBeAfter.length > 0) {
      //is it before an attribute it cannot be before? then move it down until it's not!
      var $this = $(this);
      var ok;
      do {
        ok = true;
        for (var ii = 0; ii < mustBeAfter.length; ii++) {
          if (
            $this.nextAll("*[data-name='" + mustBeAfter[ii] + "']").toArray()
              .length > 0
          ) {
  ...
  $(".xonomy .attributes").each(function() {
  ...
  $(".xonomy .element").each(function() {
    ...
    if (elSpec.displayName)
      $(this)
        .children(".tag")
        .children(".name")
        .html(
          Xonomy.textByLang(elSpec.displayName(Xonomy.harvestElement(this)))
        );
    if (elSpec.caption) {
      var jsEl = Xonomy.harvestElement(this);
      $(this)
        .children(".inlinecaption")
        .html(Xonomy.textByLang(elSpec.caption(jsEl)));
    }
          ...
          .html(
            Xonomy.textByLang(
              Xonomy.renderDisplayText(
                jsEl.getText(),
                elSpec.displayValue(jsEl)
              )
            )
          );
    }
    $(this)
      .children(".tag.opening")
      .children(".attributes")
      .children(".attribute")
      .each(function() {
        var atSpec = elSpec.attributes[this.getAttribute("data-name")];
        if (atSpec.displayName)
          $(this)
            .children(".name")
            .html(
              Xonomy.textByLang(
                atSpec.displayName(Xonomy.harvestAttribute(this))
              )
            );
        if (atSpec.displayValue)
          $(this)
            .children(".value")
            .html(
              Xonomy.textByLang(
                atSpec.displayValue(Xonomy.harvestAttribute(this))
              )
            );
        if (atSpec.caption)
          $(this)
            ...
            .html(
              "&nbsp;" +
                Xonomy.textByLang(
                  atSpec.caption(Xonomy.harvestAttribute(this))
                ) +
                "&nbsp;"
            );
      });
...
Xonomy.harvestElement = function(htmlElement, jsParent) {
  ...
  if (!Xonomy.harvestCache[htmlID]) {
    ...
    js.children = [];
    var htmlChildren = $(htmlElement)
      .children(".prominentChildren")
      .toArray()[0];
  ...
  if (js.htmlID) {
    ...
    if ($parent.toArray().length == 1) {
      ...
      for (var i = 0; i < jsParent.children.length; i++)
        if (jsParent.children[i].htmlID == js.htmlID) jsParent.children[i] = js;
    }
  }
  return jsParent;
};

Xonomy.render = function(data, editor, docSpec) {
  ...
  if (docSpec.allowLayby) {
    var laybyHtml =
      "<div class='layby closed empty' onclick='if($(this).hasClass(\"closed\")) Xonomy.openLayby()' ondragover='Xonomy.dragOver(event)' ondragleave='Xonomy.dragOut(event)' ondrop='Xonomy.drop(event)''>";
    laybyHtml +=
      "<span class='button closer' onclick='Xonomy.closeLayby();'>&nbsp;</span>";
    laybyHtml +=
      "<span class='button purger' onclick='Xonomy.emptyLayby()'>&nbsp;</span>";
...
Xonomy.renderElement = function(element) {
  ...
  if (spec.displayName)
    displayName = Xonomy.textByLang(spec.displayName(element));
  var title = "";
  ...
  html +=
    '<span class="plusminus" onclick="Xonomy.plusminus(\'' +
    htmlID +
    "')\"></span>";
  html +=
    '<span class="draghandle" draggable="true" ondragstart="Xonomy.drag(event)"></span>';
  html += "</span>";
  html +=
    '<span class="tag opening focusable" style="background-color: ' +
    spec.backgroundColour(element) +
    ';">';
  html += '<span class="punc">&lt;</span>';
  html +=
    '<span class="warner"><span class="inside" onclick="Xonomy.click(\'' +
    htmlID +
    "', 'warner')\"></span></span>";
  html +=
    '<span class="name" title="' +
    title +
    '" onclick="Xonomy.click(\'' +
    htmlID +
    "', 'openingTagName')\">" +
    displayName +
    "</span>";
  ...
  html += "</span>";
  html +=
    '<span class="rollouter focusable" onclick="Xonomy.click(\'' +
    htmlID +
    "', 'rollouter')\"></span>";
  ...
  if (!spec.oneliner(element)) {
    ...
    for (var i = 0; i < element.children.length; i++) {
      ...
      if (child.type == "element") {
        //element node
        if (
          spec.prominentChildren &&
          spec.prominentChildren(element).indexOf(child.name) > -1
        )
          html += Xonomy.renderElement(child);
      }
    }
    html += "</span>";
  }
  if (spec.caption && !spec.oneliner(element))
    html +=
      "<span class='inlinecaption'>" +
      Xonomy.textByLang(spec.caption(element)) +
      "</span>";
  html +=
    '<span class="childrenCollapsed focusable" onclick="Xonomy.plusminus(\'' +
    htmlID +
    "', true)\">&middot;&middot;&middot;</span>";
  html += '<div class="children">';
  if (spec.displayValue && !element.hasElements()) {
    html += Xonomy.renderDisplayText(
      element.getText(),
      spec.displayValue(element)
    );
  } else {
    var prevChildType = "";
    if (
      hasText &&
      (element.children.length == 0 || element.children[0].type == "element")
    ) {
    ...
    for (var i = 0; i < element.children.length; i++) {
      ...
      if (child.type == "text") html += Xonomy.renderText(child);
      //text node
    ...
    if (
      ...
      element.children[element.children.length - 1].type == "element"
    ) {
      html += Xonomy.renderText({ type: "text", value: "" }); //if inline layout, insert empty text node between two elements
    }
  }
  html += "</div>";
  html +=
    '<span class="tag closing focusable" style="background-color: ' +
    spec.backgroundColour(element) +
    ';">';
  html += '<span class="punc">&lt;</span>';
  html += '<span class="punc">/</span>';
  html +=
    '<span class="name" onclick="Xonomy.click(\'' +
    htmlID +
    "', 'closingTagName')\">" +
    displayName +
    "</span>";
...
Xonomy.renderAttribute = function(attribute, optionalParentName) {
  ...
  if (optionalParentName) {
    var spec =
      Xonomy.docSpec.elements[optionalParentName].attributes[attribute.name];
    if (spec) {
      if (spec.displayName)
        displayName = Xonomy.textByLang(spec.displayName(attribute));
      if (spec.displayValue)
        displayValue = Xonomy.textByLang(spec.displayValue(attribute));
  ...
  html +=
    ...
    attribute.name +
    '" data-value="' +
    Xonomy.xmlEscape(attribute.value) +
    '" id="' +
    htmlID +
    '" class="' +
    classNames +
  ...
  html +=
    ...
    "', 'warner')\"></span></span>";
  html +=
    '<span class="name attributeName focusable" title="' +
    title +
    '"' +
    onclick +
    ">" +
  ...
  html +=
    '<span class="valueContainer attributeValue focusable"' + onclick + ">";
  html += '<span class="punc">"</span>';
...
Xonomy.renderText = function(text) {
  ...
  html +=
    '<span class="value" onclick="Xonomy.click(\'' +
    htmlID +
    '\', \'text\')"><span class="insertionPoint"><span class="inside"></span></span><span class="dots"></span>' +
    txt +
    "</span>";
  html += "</div>";
  text.htmlID = htmlID;
...
Xonomy.chewText = function(txt) {
  ...
  for (var i = 0; i < txt.length; i++) {
    ...
    if (i == txt.length - 1 && t == " ")
      t = "<span class='space'>&middot;</span>"; //trailing space
    var id = Xonomy.nextID();
    ret +=
      "<span id='" +
      id +
      "' class='char focusable' onclick='if((event.ctrlKey||event.metaKey) && $(this).closest(\".element\").hasClass(\"hasInlineMenu\")) Xonomy.charClick(this)'>" +
      t +
...
Xonomy.charClick = function(c) {
  ...
  if (!isReadOnly) {
    ...
    if (
      ...
      $(".xonomy .char.on")
        .closest(".element")
        .is($(c).closest(".element")) //and if it has the same parent element as this character
    ) {
      ...
      for (var i = 0; i < chars.length; i++) {
        //highlight all chars between start and end
        if (i >= iFrom && i <= iTill) $(chars[i]).addClass("on");
      }
      //Save for later the info Xonomy needs to know what to wrap:
      Xonomy.textFromID = $(chars[iFrom])
        .closest(".textnode")
        .attr("id");
      Xonomy.textTillID = $(chars[iTill])
        .closest(".textnode")
        .attr("id");
      Xonomy.textFromIndex = $.inArray(
        chars[iFrom],
        $("#" + Xonomy.textFromID)
          .find(".char")
          .toArray()
      );
      Xonomy.textTillIndex = $.inArray(
        chars[iTill],
        $("#" + Xonomy.textTillID)
          .find(".char")
          .toArray()
...
Xonomy.wrap = function(htmlID, param) {
  ...
  if (Xonomy.textFromID == Xonomy.textTillID) {
    //abc --> a<XYZ>b</XYZ>c
    ...
    var txtMiddle = jsOld.value.substring(
      Xonomy.textFromIndex,
      Xonomy.textTillIndex + 1
    );
    ...
    window.setTimeout(function() {
      Xonomy.setFocus(newID, "openingTagName");
    }, 100);
  } else {
    //ab<...>cd --> a<XYZ>b<...>c</XYZ>d
    var jsOldOpen = Xonomy.harvestText(
      document.getElementById(Xonomy.textFromID)
    );
    var jsOldClose = Xonomy.harvestText(
      document.getElementById(Xonomy.textTillID)
    );
    ...
    var txtMiddleClose = jsOldClose.value.substring(
      0,
      Xonomy.textTillIndex + 1
    );
    ...
    $("#" + Xonomy.textFromID)
      .nextUntil("#" + Xonomy.textTillID)
      .each(function() {
        if ($(this).hasClass("element"))
          xml = xml.replace(
            ph,
            Xonomy.js2xml(Xonomy.harvestElement(this)) + ph
          );
        else if ($(this).hasClass("textnode"))
          xml = xml.replace(ph, Xonomy.js2xml(Xonomy.harvestText(this)) + ph);
      });
...
Xonomy.plusminus = function(htmlID, forceExpand) {
  ...
  if ($element.hasClass("collapsed")) {
    ...
    if ($element.hasClass("oneliner")) $children.fadeIn("fast");
    else $children.slideDown("fast");
  } else if (!forceExpand) {
...
Xonomy.updateCollapsoid = function(htmlID) {
  ...
  if (spec.collapsoid) {
    ...
    $element
      .find(".textnode")
      .not($element.find("> .prominentChildren *"))
      .each(function() {
        var txt = Xonomy.harvestText(this).value;
        for (var i = 0; i < txt.length; i++) {
          if (whisper.length < 35) whisper += txt[i];
          else abbreviated = true;
        }
        whisper += " ";
      });
    whisper = whisper
...
Xonomy.click = function(htmlID, what) {
  if (!Xonomy.notclick) {
    ...
    if (!isReadOnly && (what == "openingTagName" || what == "closingTagName")) {
      ...
      var surrogateElem = Xonomy.harvestElement(
    ...
    if (!isReadOnly && what == "attributeName") {
      ...
      var surrogateAttr = Xonomy.harvestAttribute(
        document.getElementById(htmlID)
      );
      ...
      if (content != "" && content != "<div class='menu'></div>") {
        ...
        Xonomy.answer = function(val) {
          ...
          var html = Xonomy.renderAttribute(
            { type: "attribute", name: name, value: val },
            elName
          );
    ...
    if (!isReadOnly && what == "text") {
      ...
      if (typeof spec.asker != "function") {
        var content = Xonomy.askLongString(
          value,
          null,
          Xonomy.harvestElement(
            $("#" + htmlID)
              .closest(".element")
              .toArray()[0]
          )
        ); //compose bubble content
      } else {
        var jsMe = Xonomy.harvestElement(
          $("#" + htmlID)
            .closest(".element")
            .toArray()[0]
        );
      ...
      if (content != "" && content != "<div class='menu'></div>") {
        ...
        Xonomy.answer = function(val) {
          ...
          $(obj).replaceWith(html);
          Xonomy.changed(
            Xonomy.harvestText(document.getElementById(jsText.htmlID))
          );
          window.setTimeout(function() {
            var id = $(html).prop("id");
            Xonomy.clickoff();
    ...
    if (what == "warner") {
      ...
      for (var iWarning = 0; iWarning < Xonomy.warnings.length; iWarning++) {
        ...
        if (warning.htmlID == htmlID) {
          content +=
            "<div class='warning'>" +
            Xonomy.formatCaption(Xonomy.textByLang(warning.text)) +
            "</div>";
        }
      }
      document.body.appendChild(Xonomy.makeBubble(content)); //create bubble
...
Xonomy.clickoff = function() {
  //event handler for the document-wide click-off event.
  if (!Xonomy.notclick) {
  ...
  $bubble.slideDown("fast", function() {
    if (Xonomy.keyNav)
      $bubble
        ...
        .first()
        .focus();
    //if the context menu contains anything with the class name 'focusme', focus it.
    else
      $bubble
        .find("input.focusme, select.focusme, textarea.focusme")
        .first()
        .focus();
  });
...
Xonomy.askString = function(defaultString, askerParameter, jsMe) {
  var width = $("body").width() * 0.4 - 75;
  var html = "";
  html += "<form onsubmit='Xonomy.answer(this.val.value); return false'>";
  html +=
    "<input name='val' class='textbox focusme' style='width: " +
    width +
    "px;' value='" +
    Xonomy.xmlEscape(defaultString) +
    "' onkeyup='Xonomy.notKeyUp=true'/>";
  html += " <input type='submit' value='OK'>";
  html += "</form>";
  return html;
};
Xonomy.askLongString = function(defaultString, askerParameter, jsMe) {
  var width = $("body").width() * 0.5 - 75;
  var html = "";
  html += "<form onsubmit='Xonomy.answer(this.val.value); return false'>";
  html +=
    "<textarea name='val' class='textbox focusme' spellcheck='false' style='width: " +
    width +
    "px; height: 150px;'>" +
    Xonomy.xmlEscape(defaultString) +
    "</textarea>";
...
Xonomy.askOpenPicklist = function(defaultString, picklist) {
  ...
  html += Xonomy.pickerMenu(picklist, defaultString);
  html +=
    "<form class='undermenu' onsubmit='Xonomy.answer(this.val.value); return false'>";
  html +=
    "<input name='val' class='textbox focusme' value='" +
    (!isInPicklist ? Xonomy.xmlEscape(defaultString) : "") +
    "' onkeyup='Xonomy.notKeyUp=true'/>";
  html += " <input type='submit' value='OK'>";
  html += "</form>";
  return html;
...
Xonomy.askRemote = function(defaultString, param, jsMe) {
  ...
  if (param.searchUrl || param.createUrl) {
    html +=
      "<form class='overmenu' onsubmit='return Xonomy.remoteSearch(\"" +
      Xonomy.xmlEscape(param.searchUrl, true) +
      '", "' +
      Xonomy.xmlEscape(param.urlPlaceholder, true) +
      '", "' +
      Xonomy.xmlEscape(Xonomy.jsEscape(defaultString)) +
      "\")'>";
    html += "<input name='val' class='textbox focusme' value=''/>";
    if (param.searchUrl)
      html +=
        " <button class='buttonSearch' onclick='return Xonomy.remoteSearch(\"" +
        Xonomy.xmlEscape(param.searchUrl, true) +
        '", "' +
        Xonomy.xmlEscape(param.urlPlaceholder, true) +
        '", "' +
    ...
    if (param.createUrl)
      html +=
        " <button class='buttonCreate' onclick='return Xonomy.remoteCreate(\"" +
        Xonomy.xmlEscape(param.createUrl, true) +
        '", "' +
        Xonomy.xmlEscape(param.searchUrl ? param.searchUrl : param.url, true) +
        '", "' +
        Xonomy.xmlEscape(param.urlPlaceholder, true) +
        '", "' +
        Xonomy.xmlEscape(Xonomy.jsEscape(defaultString)) +
  ...
  html += Xonomy.wyc(param.url, function(picklist) {
    var items = [];
    if (param.add)
      for (var i = 0; i < param.add.length; i++) items.push(param.add[i]);
...
Xonomy.remoteSearch = function(searchUrl, urlPlaceholder, defaultString) {
  ...
  $("#xonomyBubble .menu").replaceWith(
    Xonomy.wyc(searchUrl, function(picklist) {
      var items = [];
      if (text == "" && Xonomy.lastAskerParam.add)
        for (var i = 0; i < Xonomy.lastAskerParam.add.length; i++)
          items.push(Xonomy.lastAskerParam.add[i]);
      for (var i = 0; i < picklist.length; i++) items.push(picklist[i]);
      return Xonomy.pickerMenu(items, defaultString);
    })
  );
...
Xonomy.remoteCreate = function(
  ...
  if (text != "") {
    ...
    searchUrl = searchUrl.replace(urlPlaceholder, encodeURIComponent(text));
    $.ajax({ url: createUrl, dataType: "text", method: "POST" }).done(function(
      data
    ) {
...
Xonomy.pickerMenu = function(picklist, defaultString) {
  ...
  for (var i = 0; i < picklist.length; i++) {
    ...
    html +=
      "<div class='menuItem focusme techno" +
      (item.value == defaultString ? " current" : "") +
      "' tabindex='1' onclick='Xonomy.answer(\"" +
    ...
    if (item.caption != "")
      html +=
        ...
        (alone ? "alone" : "") +
        "'>" +
        Xonomy.xmlEscape(Xonomy.textByLang(item.caption)) +
        "</span>";
    html += "</div>";
  }
  html += "</div>";
...
Xonomy.wyc = function(url, callback) {
  //a "when-you-can" function for delayed rendering: gets json from url, passes it to callback, and delayed-returns html-as-string from callback
  Xonomy.wycLastID++;
  var wycID = "xonomy_wyc_" + Xonomy.wycLastID;
  if (Xonomy.wycCache[url]) return callback(Xonomy.wycCache[url]);
  Xonomy.wycQueue.push(function() {
    //push job to WYC queue
    ...
    if (Xonomy.wycCache[url]) {
      ...
      if (Xonomy.wycQueue.length > 0) Xonomy.wycQueue[0]();
      else Xonomy.wycIsRunning = false; //run the next WYC job, or say that WYC has finished running
    } else {
      $.ajax({ url: url, dataType: "json", method: "POST" }).done(function(
        data
      ) {
...
Xonomy.internalMenu = function(htmlID, items, harvest, getter, indices) {
  ...
  var fragments = items.map(function(item, i) {
    ...
    if (includeIt) {
      ...
      var icon = "";
      if (item.icon)
        icon = "<span class='icon'><img src='" + item.icon + "'/></span> ";
      var key = "";
      if (item.keyTrigger && item.keyCaption)
        key =
          "<span class='keyCaption'>" +
          Xonomy.textByLang(item.keyCaption) +
          "</span>";
      if (item.menu) {
        var internalHtml = Xonomy.internalMenu(
          htmlID,
          item.menu,
          harvest,
          getter,
        ...
        if (internalHtml != "<div class='submenu'></div>") {
          html +=
            "<div class='menuItem" +
            (item.expanded(jsMe) ? " expanded" : "") +
            "'>";
          html +=
            "<div class='menuLabel focusme' tabindex='0' onkeydown='if(Xonomy.keyNav && [37, 39].indexOf(event.which)>-1) Xonomy.toggleSubmenu(this.parentNode)' onclick='Xonomy.toggleSubmenu(this.parentNode)'>" +
            icon +
            Xonomy.formatCaption(Xonomy.textByLang(item.caption(jsMe))) +
            "</div>";
          html += internalHtml;
          html += "</div>";
        }
        ...
        html +=
          "<div class='menuItem focusme' tabindex='0' onclick='Xonomy.callMenuFunction(" +
          getter(indices) +
          ', "' +
          htmlID +
          "\")'>";
...
Xonomy.attributeMenu = function(htmlID) {
  ...
  var elName = $("#" + htmlID)
    .closest(".element")
    .attr("data-name"); //obtain element's name
  Xonomy.verifyDocSpecAttribute(elName, name);
  var spec = Xonomy.docSpec.elements[elName].attributes[name];
  ...
  return Xonomy.internalMenu(
    htmlID,
    spec.menu,
    Xonomy.harvestAttribute,
    getter
  );
};
...
Xonomy.formatCaption = function(caption) {
  caption = caption.replace(
    /\<(\/?)([^\>\/]+)(\/?)\>/g,
    "<span class='techno'><span class='punc'>&lt;$1</span><span class='elName'>$2</span><span class='punc'>$3&gt;</span></span>"
  );
  caption = caption.replace(
    /\@"([^\"]+)"/g,
  ...
  caption = caption.replace(
    /\@([^ =]+)=""/g,
    "<span class='techno'><span class='atName'>$1</span><span class='punc equals'>=</span><span class='punc'>\"</span><span class='punc'>\"</span></span>"
  );
  caption = caption.replace(
    /\@([^ =]+)="([^\"]+)"/g,
    "<span class='techno'><span class='atName'>$1</span><span class='punc equals'>=</span><span class='punc'>\"</span><span class='atValue'>$2</span><span class='punc'>\"</span></span>"
  );
  caption = caption.replace(
    /\@([^ =]+)/g,
    "<span class='techno'><span class='atName'>$1</span></span>"
  );
  return caption;
};
...
Xonomy.newAttribute = function(htmlID, parameter) {
  ...
  var $element = $("#" + htmlID);
  var html = Xonomy.renderAttribute(
    { type: "attribute", name: parameter.name, value: parameter.value },
    $element.data("name")
  );
  ...
  if (
    $("#" + htmlID + " > .tag.opening > .attributes")
      .children("[data-name='" + parameter.name + "'].shy")
      .toArray().length > 0
  ) {
    if (
      !$("#" + htmlID)
  ...
  if (parameter.value == "") Xonomy.click($(html).prop("id"), "attributeValue");
...
Xonomy.elementReorder = function(htmlID) {
  ...
  if (elSpec.mustBeBefore) {
    //is it after an element it cannot be after? then move it up until it's not!
    var $this = $(that);
  ...
  if (elSpec.mustBeAfter) {
    //is it before an element it cannot be before? then move it down until it's not!
    var $this = $(that);
    var jsElement = Xonomy.harvestElement(that);
...
Xonomy.editRaw = function(htmlID, parameter) {
  ...
  else if (parameter.fromXml)
    var txt = parameter.fromXml(Xonomy.js2xml(jsElement));
  ...
  Xonomy.answer = function(val) {
    ...
    else if (parameter.toXml)
      jsNewElement = Xonomy.xml2js(
        parameter.toXml(val, jsElement),
        jsElement.parent()
      );
...
Xonomy.duplicateElement = function(htmlID) {
  ...
  html = html.replace(/ id=['"]/g, function(x) {
    return x + prefixID + "_";
  });
  html = html.replace(/Xonomy\.click\(['"]/g, function(x) {
    return x + prefixID + "_";
  });
  html = html.replace(/Xonomy\.plusminus\(['"]/g, function(x) {
    return x + prefixID + "_";
  });
  var $html = $(html).hide();
  $("#" + htmlID).after($html);
...
Xonomy.moveElementUp = function(htmlID) {
  ...
  window.setTimeout(function() {
    Xonomy.setFocus(htmlID, "openingTagName");
  }, 100);
};
...
Xonomy.mergeElements = function(elDead, elLive) {
  ...
  if (elLive && elLive.type == "element") {
    for (var i = 0; i < elDead.attributes.length; i++) {
      ...
      if (
        !elLive.hasAttribute(atDead.name) ||
        elLive.getAttributeValue(atDead.name) == ""
      ) {
        elLive.setAttribute(atDead.name, atDead.value);
        if (elLive.hasAttribute(atDead.name))
          $("#" + elLive.getAttribute(atDead.name).htmlID).remove();
        $("#" + elLive.htmlID)
          .find(".attributes")
          .first()
          .append($("#" + elDead.attributes[i].htmlID));
      }
    }
    var specDead = Xonomy.docSpec.elements[elDead.name];
    var specLive = Xonomy.docSpec.elements[elLive.name];
    if (specDead.hasText(elDead) || specLive.hasText(elLive)) {
      //if either element is meant to have text, concatenate their children
      if (elLive.getText() != "" && elDead.getText() != "") {
        elLive.addText(" ");
        $("#" + elLive.htmlID)
          .find(".children")
          .first()
          .append(Xonomy.renderText({ type: "text", value: " " }));
      }
      for (var i = 0; i < elDead.children.length; i++) {
        ...
        $("#" + elLive.htmlID)
          .find(".children")
          .first()
          .append($("#" + elDead.children[i].htmlID));
      }
    } else {
      //if no text, merge their children one by one
      for (var i = 0; i < elDead.children.length; i++) {
...
Xonomy.deleteEponymousSiblings = function(htmlID, parameter) {
  ...
  var _htmlChildren = $(parent)
    .children(".children")
    .toArray()[0].childNodes;
  var htmlChildren = [];
  for (var i = 0; i < _htmlChildren.length; i++)
    htmlChildren.push(_htmlChildren[i]);
  for (var i = 0; i < htmlChildren.length; i++) {
    var htmlChild = htmlChildren[i];
    if ($(htmlChild).hasClass("element")) {
      if (
        $(htmlChild).attr("data-name") == $(obj).attr("data-name") &&
        htmlChild != obj
      ) {
...
Xonomy.insertDropTargets = function(htmlID) {
  ...
  var elSpec = Xonomy.docSpec.elements[elementName];
  $(".xonomy .element:visible > .children").append(
  ...
  $(".xonomy .element:visible > .children > .element").before(
    "<div class='elementDropper' ondragover='Xonomy.dragOver(event)' ondragleave='Xonomy.dragOut(event)' ondrop='Xonomy.drop(event)'><div class='inside'></div></div>"
  );
  $(".xonomy .element:visible > .children > .text").before(
    "<div class='elementDropper' ondragover='Xonomy.dragOver(event)' ondragleave='Xonomy.dragOut(event)' ondrop='Xonomy.drop(event)'><div class='inside'></div></div>"
  );
  $(".xonomy .dragging .children:visible > .elementDropper").remove(); //remove drop targets fom inside the element being dragged
  $(".xonomy .dragging")
    .prev(".elementDropper")
    .remove(); //remove drop targets from immediately before the element being dragged
  $(".xonomy .dragging")
    .next(".elementDropper")
    .remove(); //remove drop targets from immediately after the element being dragged
  ...
  var harvestElement = function(div) {
    ...
    if (!harvestCache[htmlID])
      harvestCache[htmlID] = Xonomy.harvestElement(div);
    return harvestCache[htmlID];
  };

  if (elSpec.localDropOnly(harvestElement($element.toArray()[0]))) {
    if (elSpec.canDropTo) {
      //remove the drop target from elements that are not the dragged element's parent
  ...
  if (elSpec.canDropTo) {
    //remove the drop target from elements it cannot be dropped into
    var droppers = $(".xonomy .elementDropper").toArray();
    for (var i = 0; i < droppers.length; i++) {
      var dropper = droppers[i];
      var parentElementName = $(dropper.parentNode.parentNode)
        .toArray()[0]
  ...
  if (elSpec.mustBeBefore) {
    //remove the drop target from after elements it cannot be after
    ...
    for (var i = 0; i < droppers.length; i++) {
      ...
      for (var ii = 0; ii < mustBeBefore.length; ii++) {
        if (
          $(dropper)
            .prevAll("*[data-name='" + mustBeBefore[ii] + "']")
            .toArray().length > 0
        ) {
          dropper.parentNode.removeChild(dropper);
        }
  ...
  if (elSpec.mustBeAfter) {
    //remove the drop target from before elements it cannot be before
    var jsElement = harvestElement($element.toArray()[0]);
    var droppers = $(".xonomy .elementDropper").toArray();
    for (var i = 0; i < droppers.length; i++) {
      var dropper = droppers[i];
      ...
      for (var ii = 0; ii < mustBeAfter.length; ii++) {
        if (
          $(dropper)
            .nextAll("*[data-name='" + mustBeAfter[ii] + "']")
            .toArray().length > 0
        ) {
          dropper.parentNode.removeChild(dropper);
        }
...
Xonomy.openCloseLayby = function() {
  //open the layby if it's full, close it if it's empty
...
Xonomy.changed = function(jsElement) {
...
Xonomy.startKeyNav = function(keyboardEventCatcher, scrollableContainer) {
  ...
  $(document).on("keydown", function(e) {
    if (
      ...
      $("input:focus, select:focus, textarea:focus").length == 0
    )
      e.preventDefault();
...
Xonomy.setFocus = function(htmlID, what) {
  if (Xonomy.keyNav) {
    ...
    if (Xonomy.currentFocus == "childrenCollapsed")
      $("#" + htmlID + " > .childrenCollapsed")
        .last()
        .addClass("focused");
    if (Xonomy.currentFocus == "rollouter")
      $("#" + htmlID + " > .tag.opening > .rollouter")
        .last()
        .addClass("focused");
  }
};
  ...
  if (!Xonomy.notKeyUp) {
    if (!event.shiftKey && !$("#xonomyBubble").length > 0) {
      if (event.which == 27) {
        ...
        event.stopImmediatePropagation();
        if (Xonomy.currentFocus == "childrenCollapsed")
          Xonomy.plusminus(Xonomy.currentHtmlId, true);
        ...
        //down key with Ctrl or Cmd (Mac OS)
        event.preventDefault();
        event.stopImmediatePropagation();
        Xonomy.scrollableContainer.scrollTop(
          Xonomy.scrollableContainer.scrollTop() + 60
        );
      } else if ((event.ctrlKey || event.metaKey) && event.which == 38) {
        //up key with Ctrl or Cmd (Mac OS)
        event.preventDefault();
        event.stopImmediatePropagation();
        Xonomy.scrollableContainer.scrollTop(
          Xonomy.scrollableContainer.scrollTop() - 60
        ...
        [37, 39].indexOf(event.which) > -1
      ) {
        //arrow keys with Ctrl or Cmd (Mac OS)
        event.preventDefault();
        event.stopImmediatePropagation();
        var $el = $("#" + Xonomy.currentHtmlId);
        if ($el.hasClass("element") && !$el.hasClass("uncollapsible")) {
          if (event.which == 39 && $el.hasClass("collapsed")) {
            //expand it!
            Xonomy.plusminus(Xonomy.currentHtmlId);
          }
          if (event.which == 37 && !$el.hasClass("collapsed")) {
            //collapse it!
            Xonomy.plusminus(Xonomy.currentHtmlId);
          }
        }
      } else if ([37, 38, 39, 40].indexOf(event.which) > -1 && !event.altKey) {
        //arrow keys
        event.preventDefault();
        event.stopImmediatePropagation();
        if (!Xonomy.currentHtmlId) {
        ...
        } else if ($(".xonomy .focused").length == 0) {
          //something is current but nothing is focused yet
          Xonomy.setFocus(Xonomy.currentHtmlId, Xonomy.currentFocus);
        } else {
          //something is current, do arrow action
...
(Xonomy.keyboardMenu = function(event) {
  ...
  (Xonomy.goDown = function() {
    if (
      Xonomy.currentFocus != "openingTagName" &&
      Xonomy.currentFocus != "closingTagName" &&
      Xonomy.currentFocus != "text" &&
      Xonomy.currentFocus != "char"
    ) {
      Xonomy.goRight();
    } else {
      var $el = $("#" + Xonomy.currentHtmlId);
      var $me = $el;
      if (Xonomy.currentFocus == "openingTagName")
        var $me = $el.find(".tag.opening").first();
      if (Xonomy.currentFocus == "closingTagName")
        var $me = $el.find(".tag.closing").last();

      var $candidates = $(".xonomy .focusable:visible")
        .not(".attributeName")
        .not(".attributeValue")
        .not(".childrenCollapsed")
        .not(".rollouter");
      $candidates = $candidates.not(".char").add($el);
      if (Xonomy.currentFocus == "openingTagName" && $el.hasClass("oneliner"))
        $candidates = $candidates
          .not("#" + Xonomy.currentHtmlId + " .tag.closing")
          .not("#" + Xonomy.currentHtmlId + " .children *");
      if (Xonomy.currentFocus == "openingTagName" && $el.hasClass("oneliner"))
        $candidates = $candidates.not(
          "#" + Xonomy.currentHtmlId + " .textnode"
        );
      if (Xonomy.currentFocus == "openingTagName")
        $candidates = $candidates.not(".prominentChildren *");
      if ($el.hasClass("collapsed"))
        $candidates = $candidates.not(
          "#" + Xonomy.currentHtmlId + " .tag.closing"
        );
      if (
        $el.hasClass("textnode") &&
        $el.closest(".prominentChildren").length == 0 &&
        $(".xonomy").hasClass("nerd")
      )
        var $candidates = $el
          .closest(".element")
          .find(".tag.closing:visible")
          .last();
      if ($el.hasClass("textnode") && $(".xonomy").hasClass("laic"))
        var $candidates = $el
          .closest(".element")
          .next()
          .find(".focusable:visible")
          .first();

      var $next = $candidates.eq($candidates.index($me[0]) + 1);
      if ($next.hasClass("opening"))
        Xonomy.setFocus($next.closest(".element").prop("id"), "openingTagName");
      else if ($next.hasClass("closing"))
        Xonomy.setFocus($next.closest(".element").prop("id"), "closingTagName");
      else if ($next.hasClass("textnode"))
        Xonomy.setFocus($next.prop("id"), "text");
    }
  });
Xonomy.goUp = function() {
  if (
    ...
    if (
      ...
      $candidates = $candidates.add(".xonomy .prominentChildren .textnode");
    if (
      $el.hasClass("textnode") &&
      $el.closest(".prominentChildren").length == 0
    )
      $candidates = $el
        ...
        .first()
        .add($el);
    if ($me.hasClass("closing") && $el.hasClass("hasText"))
      $candidates = $candidates.not(
        "#" + Xonomy.currentHtmlId + " .children *:not(:first-child)"
      );
    if (
      $me.hasClass("opening") &&
      $el
      ...
      var siblingID = $el
        ...
        .prop("id");
      $candidates = $candidates.not(
        "#" + siblingID + " .children *:not(:first-child)"
      );
    }
...
Xonomy.goRight = function() {
  ...
  if (Xonomy.currentFocus == "attributeName")
    var $me = $el.find(".attributeName").first();
  if (Xonomy.currentFocus == "attributeValue")
    var $me = $el.find(".attributeValue").first();
  if (Xonomy.currentFocus == "childrenCollapsed")
    var $me = $el
      .find(".childrenCollapsed")
      .not(".prominentChildren *")
      .first();
  if (Xonomy.currentFocus == "rollouter")
    var $me = $el.find(".rollouter").first();

  var $candidates = $(".xonomy .focusable:visible");
  $candidates = $candidates
    .not(".char")
    .add(".hasInlineMenu > .children > .textnode .char:visible");

  var $next = $candidates.eq($candidates.index($me[0]) + 1);
  if ($next.hasClass("attributeName"))
    Xonomy.setFocus($next.closest(".attribute").prop("id"), "attributeName");
  if ($next.hasClass("attributeValue"))
    Xonomy.setFocus($next.closest(".attribute").prop("id"), "attributeValue");
  if ($next.hasClass("opening"))
    Xonomy.setFocus($next.closest(".element").prop("id"), "openingTagName");
  if ($next.hasClass("closing"))
    Xonomy.setFocus($next.closest(".element").prop("id"), "closingTagName");
  if ($next.hasClass("textnode")) Xonomy.setFocus($next.prop("id"), "text");
  if ($next.hasClass("childrenCollapsed"))
    Xonomy.setFocus($next.closest(".element").prop("id"), "childrenCollapsed");
  if ($next.hasClass("rollouter"))
    Xonomy.setFocus($next.closest(".element").prop("id"), "rollouter");
